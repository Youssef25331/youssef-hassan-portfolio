...
export default function Hero() {
  return (
    <div className="h-[100vh] w-full flex justify-center content-center overflow- z-10 min-h-[30rem]">
      <div className="relative -z-1 h-full w-full bg-base-100">
        <div className="absolute inset-0"
          style={{ width: "100vw", background: "radial-gradient(circle at top,hsl(from var(--color-primary) h s 12%) 0%,var(--color-base-100) 100vh,transparent 100%)" }}
        />
        <div
          className="z-2 absolute inset-0 pointer-events-none isolate opacity-30 contain-strict hidden lg:block">
        </div>
      </div>
      <div className="h-[80rem] w-[35vh] -translate-y-[350px] absolute left-0 top-0 -rotate-45 rounded-full bg-[radial-gradient(68.54%_68.72%_at_55.02%_31.46%,hsla(0,0%,85%,.08)_0,hsla(0,0%,55%,.02)_50%,hsla(0,0%,45%,0)_80%)]" />
      <div className="h-[80vh] w-56 absolute left-0 top-0  -rotate-45 rounded-full bg-[radial-gradient(50%_50%_at_50%_50%,hsla(0,0%,85%,.06)_0,hsla(0,0%,45%,.02)_80%,transparent_100%)] [translate:5%_-50%]" />
      <div className="h-[80vh] w-56 -translate-y-[350px] absolute left-0 top-0  -rotate-45 bg-[radial-gradient(50%_50%_at_50%_50%,hsla(0,0%,85%,.04)_0,hsla(0,0%,45%,.02)_80%,transparent_100%)]" />
      <Sparkles
        density={1200}
